...
void updateLeaderboard() {
    ...
    time_t now = time(0);
    struct tm ltm_data;//mini - container that holds time broken into pieces //itm_creates a variable
    localtime_s(&ltm_data, &now);
    char dateTimeStr[80];
    strftime(dateTimeStr, 80, "%c", &ltm_data);//Converts the struct tm time into a readable text format
    ...
    while (getline(file, line) && count < 100) {
        ...
        if (secondPipe != string::npos) {
            ...
            if (thirdPipe != string::npos) {
                scoreStr = line.substr(secondPipe + 1, thirdPipe - secondPipe - 1);//find() = searches for a character
            }
            else {
